...
class NewsStory:
   ...
   def setDate(self, date):    self._date = Date(date)
   def setBody(self, line):    self._body = [line]
   def appendBody(self, line): self._body.append(line)
   ...
   def getSummaryRectangle(self):
      ...
      return StoryRectangle([
         H1(self.getTitle()),
         "".join([str(para) for para in wikiHtml])
      ])
