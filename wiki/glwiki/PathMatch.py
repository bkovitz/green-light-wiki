...
class PathMatch:
    ...
    def pathInfo(self, path, httpHost=None):
        ...
        for pathLine in self._pathsFile:
            hostName, wikiName, fileType, pathWithWildcards = _parsePathLine(pathLine)
            ...
            if fileType:
                ...
                defaultPage = _appendSlash(path) + _defaultPageForFileType(
                    fileType, wikiName
                )
                if (
                ...
                if (
                    pathWithWildcards == "*"
                    and re.match(regexp, defaultPage)
                    # and
                    # os.path.exists(config.getNonwikiFilename(defaultPage))
                ):
                    return wikiName, fileType.upper(), defaultPage
