   ...
   def _renderItems(self, indent, items):
      ...
      for item in items:
         result += self._renderItem(
            item,
            indent,
            isFirstOnLine and self.indentsChildren
         )
...
class HtmlAnchor(Container):
   ...
   def _preprocess(self):
      return (
         [HtmlQuotedAttribute("HREF", self.href)] + self.attrs
         ,
         self.items
      )
...
class HtmlImage(Standalone):
   ...
   def __init__(self, src):
      Standalone.__init__(self,
         "IMG", 
         [
            HtmlQuotedAttribute("SRC", src),
            HtmlAttribute("BORDER", 0)
         ]
      )
