...
class WikiText2:
    ...
    def renderParas(self, page):
        return [para.makeChunk(page) for para in self.makeParas()]

    def makeParas(self):
        initialParas = [self._makePara(para) for para in _parseParas(self.text)]
    ...
    def _makePara(self, para):
        if para.__class__ == LineBreak:
            ...
            return Paragraph(
                self._headingChunk(para), self._renderWikiText(self._headingText(para))
            )
        elif self._isUnorderedListItem(para):
            return Paragraph(UnorderedListItemChunk, self._renderWikiText(para[2:]))
        elif self._isOrderedListItem(para):
            return Paragraph(OrderedListItemChunk, self._renderWikiText(para[2:]))
        else:
            return Paragraph(ParagraphChunk, self._renderWikiText(para))
