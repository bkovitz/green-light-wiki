...
latexTemplate = \
r"""\documentclass[10pt,notitlepage]{article}
\usepackage{amsmath}
\usepackage{amsfonts}
\usepackage[all]{xy}
\begin{document}
...
insecureCommands = [ re.compile(s) for s in [
   "newcommand",
   "renewcommand",
   "providecommand",
   "newenviroment",
...
class LatexImage:
   ...
   def _makeCommands(self, imageFilename):
      return [
         ...
         ShellCommand("gs -dSAFER -dDOINTERPOLATE -dTextAlphaBits=4 -dGraphicsAlphaBits=4 -dNOPAUSE -r110x110 -sDEVICE=pnggray -sOutputFile=expr.png expr.pdf"),
   ...
   def _makeImg(self, imageFilename):
      return (
         '<IMG CLASS="latex-image" SRC="%s">'
         %
         config.makeUrl(self._repository.getWikiName(), imageFilename)
      )
