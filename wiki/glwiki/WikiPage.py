...
class WikiPage(RequestedPage):
    ...
    def top(self):
        return TopChunk(self, self.getTitle(), self.homeLink(), self.logo())

    def logo(self):
        logo = config.logo(self.getWikiName())
        if logo:
            return HtmlPara(
                HtmlAnchor(config.homeUrl(self.getWikiName()), HtmlImage(logo)),
                Class("logo"),
            )
    ...
    def getPageUrl(self):
        return config.makeUrl(self.getWikiName(), self.getPageName())
    ...
    def getSessionId(self):
        ...
        if not sessionId:
            sessionId = SessionDatabase.makeSession(
                int(time()), self._environment.getIpAddress()
            )
