...
class SavePage(WikiPage):
    ...
    def _setPendingMessage(self, messageChunkClass):
        SessionDatabase.setPendingMessage(
            self.getSessionId(), messageChunkClass, (self.getTitle(),)
        )
...
"""
    ...
    def getMessage(self):
        url = config.makeUrl(self.getWikiName(), self.getPageName())

        if self.savedSuccessfully():
            return None
        else:
            return CouldntSaveDueToConflictChunk(
                self, self.getPageName(), self._previousAuthor(), self.getRepository()
            )
    ...
    def _save(self):
        ...
        RecentChanges.logChange2(
            self.getWikiName(), self.getUserName(), self.getPageName()
        )
